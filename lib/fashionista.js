...
function fashionista(options) {
  ...
  if (_.pluck(options.themes, 'name').indexOf('foundation') === -1) {
...
fashionista.prototype.decorate = function(app) {
  var themes = {};
  var defaultTheme = this.options.themes[0].theme;
  ...
  app.use(this.options.path, function(req, res, next) {
    if (req.url === '/') {
      ...
      res.send('var Fashionista = ' + client + 'var fashionista = new Fashionista(' + JSON.stringify(themes) + ');fashionista.use("' + defaultTheme + '");');
    } else {
      next();
    }
  });
